...
let QueryService = class QueryService {
    ...
    buildCandidates({ chunks, lowerQuestion, tokens, questionEmbedding, limit, }) {
        return chunks
            .map((chunk) => {
    ...
    looksLikeRiskTitle(value, question) {
        const trimmed = value.trim();
        if (!trimmed)
            return true;
        if (trimmed === question.trim())
            return true;
        if (trimmed.length <= 3)
            return true;
    ...
    generateEvidenceSuggestions(riskPoint) {
        const suggestions = {
            '采购程序不规范': ['调取采购申请、审批单据', '核查招标文件、比价记录', '检查采购合同与执行情况'],
            '供应商遴选不透明': ['获取供应商准入资料', '核查遴选评审记录', '检查关联方关系披露'],
            '验收控制流于形式': ['调取验收单据、签字记录', '核对实物与合同约定', '检查质量检测报告'],
            '采购价格异常': ['获取市场询价记录', '比对同类商品价格', '检查价格审批依据'],
            '收入确认不准确': ['调取销售合同、发货单', '核对收入确认时点', '检查收入会计处理'],
            '异常销售交易未被识别': ['获取年末销售明细', '核查关联方交易', '检查退货记录'],
            '回款管理失控': ['调取应收账款明细', '核对银行回款记录', '检查账龄分析表'],
            '退货折让处理不规范': ['获取退货审批单据', '核对退货入库记录', '检查会计处理凭证'],
            '存货账实不符': ['调取盘点表、差异说明', '核对账面与实物数量', '检查调整处理依据'],
            '跌价减值识别不充分': ['获取存货库龄分析', '核查呆滞存货清单', '检查减值计提依据'],
            '出入库控制薄弱': ['调取出入库单据', '核对审批授权记录', '检查库存台账'],
            '异常损耗未被及时发现': ['获取损耗报废记录', '核查审批流程', '检查处置依据'],
            '资金收付审批失控': ['调取付款申请、审批单', '核对银行流水', '检查授权审批制度执行'],
            '银行账户管理不规范': ['获取银行账户清单', '核查开户审批手续', '检查账户使用记录'],
            '现金管理存在漏洞': ['调取现金日记账', '核对库存现金盘点', '检查现金收支凭证'],
            '账实核对不及时': ['获取银行对账单', '核查余额调节表', '检查未达账项处理'],
            '费用报销不合规': ['调取报销单据、发票', '核对审批流程', '检查费用标准执行'],
            '费用归属期间不准确': ['获取费用明细账', '核查跨期费用', '检查会计期间划分'],
            '审批和预算约束失效': ['调取预算执行报告', '核对超预算审批', '检查预算调整依据'],
            '异常费用支出未被识别': ['获取大额费用明细', '核查异常支出事项', '检查专项审批记录'],
            '关键业务程序执行不规范': ['调取业务流程文件', '核对实际执行记录', '检查例外事项处理'],
            '职责分离或授权审批不到位': ['获取岗位职责说明', '核查授权审批矩阵', '检查不相容职务分离'],
            '业务资料记录不完整': ['调取业务台账、凭证', '核对原始单据完整性', '检查归档管理情况'],
            '异常交易或数据变动未被及时识别': ['获取交易明细数据', '核查异常波动事项', '检查监控预警记录'],
        };
        return suggestions[riskPoint] || ['调取原始业务资料', '核对审批流程与执行记录', '比对台账、单据与实际执行情况'];
    }
    generatePossibleFindings(riskPoint) {
        const findings = {
            '采购程序不规范': ['采购未履行规定审批程序', '存在拆分采购规避监管', '采购文件不完整或缺失'],
            '供应商遴选不透明': ['供应商准入缺乏公开程序', '存在关联方利益输送', '供应商资质审查不严'],
            '验收控制流于形式': ['验收流于形式未实质审查', '验收记录与实际不符', '存在虚假验收签字'],
            '采购价格异常': ['采购价格明显高于市场价', '缺乏比价询价程序', '存在价格舞弊风险'],
            '收入确认不准确': ['收入确认时点不符合准则', '虚构交易虚增收入', '跨期确认调节利润'],
            '异常销售交易未被识别': ['年末突击销售异常', '关联方交易未披露', '存在虚假销售退货'],
            '回款管理失控': ['应收账款长期挂账', '回款记录与银行不符', '存在资金被挪用风险'],
            '退货折让处理不规范': ['退货未及时入账', '折让缺乏审批依据', '影响收入真实性'],
            '存货账实不符': ['盘点发现账实差异', '存在存货流失或虚增', '盘点程序不规范'],
            '跌价减值识别不充分': ['呆滞存货未计提减值', '存货价值虚高', '减值测试不充分'],
            '出入库控制薄弱': ['出入库缺乏审批', '存在无单出库', '库存记录不准确'],
            '异常损耗未被及时发现': ['损耗报废缺乏依据', '存在资产流失', '处置程序不规范'],
            '资金收付审批失控': ['付款缺乏有效审批', '存在越权支付', '资金被挪用侵占'],
            '银行账户管理不规范': ['存在账外账户', '私设小金库', '账户管理混乱'],
            '现金管理存在漏洞': ['现金坐支或白条抵库', '库存现金超限额', '现金管理不规范'],
            '账实核对不及时': ['银行对账不及时', '未达账项长期挂账', '存在资金异常'],
            '费用报销不合规': ['报销票据不真实', '费用超标准支出', '存在虚假报销'],
            '费用归属期间不准确': ['费用跨期调节利润', '费用资本化不当', '会计处理不准确'],
            '审批和预算约束失效': ['超预算支出未审批', '预算约束形同虚设', '费用控制失效'],
            '异常费用支出未被识别': ['大额费用缺乏依据', '存在违规支出', '可能存在利益输送'],
            '关键业务程序执行不规范': ['业务流程执行不到位', '存在规避程序', '内控执行失效'],
            '职责分离或授权审批不到位': ['不相容职务未分离', '一人经办到底', '授权审批失控'],
            '业务资料记录不完整': ['业务台账记录缺失', '原始凭证不完整', '影响业务可追溯性'],
            '异常交易或数据变动未被及时识别': ['异常交易未被监控', '数据波动缺乏解释', '可能掩盖舞弊或错误'],
        };
        return findings[riskPoint] || ['可能存在制度执行不到位', '可能存在内控缺失、程序不规范或异常事项未被识别'];
    }
    generateRectificationSuggestions(riskPoint) {
        const suggestions = {
            '采购程序不规范': ['完善采购管理制度', '严格执行审批程序', '加强采购过程监督'],
            '供应商遴选不透明': ['建立供应商准入机制', '公开遴选评审程序', '加强关联方审查'],
            '验收控制流于形式': ['强化验收责任制', '完善验收记录', '加强质量检查'],
            '采购价格异常': ['建立比价询价机制', '加强价格审核', '定期市场价格调查'],
            '收入确认不准确': ['规范收入确认标准', '加强会计核算监督', '完善收入审核程序'],
            '异常销售交易未被识别': ['建立异常交易监控', '加强关联方管理', '完善退货审批'],
            '回款管理失控': ['加强应收账款管理', '定期对账催收', '建立坏账预警机制'],
            '退货折让处理不规范': ['规范退货审批流程', '及时入账处理', '加强退货管理'],
            '存货账实不符': ['定期盘点核对', '查明差异原因', '完善库存管理'],
            '跌价减值识别不充分': ['建立减值测试机制', '及时计提减值', '加强存货管理'],
            '出入库控制薄弱': ['完善出入库审批', '加强库存记录', '定期盘点核对'],
            '异常损耗未被及时发现': ['规范报废审批程序', '加强资产监管', '完善处置流程'],
            '资金收付审批失控': ['完善资金审批制度', '严格授权管理', '加强资金监控'],
            '银行账户管理不规范': ['清理规范账户', '加强账户监管', '完善开户审批'],
            '现金管理存在漏洞': ['规范现金管理', '加强库存现金监督', '完善现金收支审批'],
            '账实核对不及时': ['定期银行对账', '及时处理未达账项', '加强资金监控'],
            '费用报销不合规': ['完善报销审批制度', '加强票据审核', '规范费用标准'],
            '费用归属期间不准确': ['规范会计核算', '加强跨期费用管理', '完善会计监督'],
            '审批和预算约束失效': ['强化预算约束', '规范超预算审批', '加强预算执行监督'],
            '异常费用支出未被识别': ['建立费用监控机制', '加强大额支出审核', '完善专项审批'],
            '关键业务程序执行不规范': ['完善业务流程制度', '加强执行监督', '定期检查评估'],
            '职责分离或授权审批不到位': ['明确岗位职责', '完善授权审批', '加强不相容职务分离'],
            '业务资料记录不完整': ['完善台账记录', '规范资料归档', '加强资料管理'],
            '异常交易或数据变动未被及时识别': ['建立监控预警机制', '加强异常事项审查', '完善风险管理'],
        };
        return suggestions[riskPoint] || ['完善制度执行流程', '补齐审批、验收和归档资料', '强化关键岗位复核与监督机制'];
    }
    async assessRiskLevels(question, riskItems) {
...
数组长度必须等于风险点数量（${riskItems.length}个），每个元素只能是"高"、"中"或"低"。`;
        try {
            ...
            return riskItems.map((item, index) => {
                ...
                if (highKeywords.some(kw => riskPoint.includes(kw)))
                    return '高';
                if (lowKeywords.some(kw => riskPoint.includes(kw)))
                    return '低';
                return index < 3 ? '高' : index < 6 ? '中' : '低';
            });
        }
    }
    async buildFallbackRiskTable(question, citations, similarCases, userTier) {
        ...
        const rows = availableCitations.map((citation, index) => {
            ...
            return {
                ...
                detail: {
                    ...
                    evidenceSuggestions: this.generateEvidenceSuggestions(riskPoint),
                    possibleFindings: this.generatePossibleFindings(riskPoint),
                    rectificationSuggestions: this.generateRectificationSuggestions(riskPoint),
                },
            };
        });
        return {
    ...
    async search(dto, options) {
        ...
        const userTier = plan.id === 'free' ? 'free' : 'subscribed';
        ...
        const baseRegulationTypes = ['regulation'];
        const localTypes = ['local_policy'];
        const caseTypes = canAccessCases ? ['national_case', 'local_case'] : [];
        const privateTypes = resolvedGroupId ? ['private'] : [];
        const industryTypes = ['industry'];
