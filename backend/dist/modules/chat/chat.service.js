...
let ChatService = class ChatService {
    ...
    removeGroupConversations(groupId) {
        const removedConversationIds = new Set(this.conversations.filter((conversation) => conversation.groupId === groupId).map((conversation) => conversation.id));
        this.conversations.splice(0, this.conversations.length, ...this.conversations.filter((conversation) => !removedConversationIds.has(conversation.id)));
        this.messages.splice(0, this.messages.length, ...this.messages.filter((message) => !removedConversationIds.has(message.conversationId)));
        this.persistState();
    }
    syncGroupAgent(group, agent) {
